...
class MinimalTaskExecutorSlotsFilter implements SlotsFilter {
    ...
    MinimalTaskExecutorSlotsFilter(boolean localityPreferred) {
        ...
        return slotsByTaskExecutor.keySet().stream()
                .sorted(
                        Comparator.comparingInt(
                                resourceID -> slotsByTaskExecutor.get(resourceID).size()))
    ...
    private static List<ResourceID> getLocalityPreferredOrderedTaskExecutors(
        ...
        Map<ResourceID, Long> resourceScores =
                getScorePerTaskExecutor(scores, slotsByTaskExecutor, allocationIdToResourceId);
        return slotsByTaskExecutor.keySet().stream()
                .sorted(
                        (left, right) -> {
                            int diff =
    ...
    private static Map<ResourceID, Long> getScorePerTaskExecutor(
            Collection<AllocationScore> scores,
            Map<ResourceID, ? extends Set<? extends SlotInfo>> slotsByTaskExecutor,
            Map<AllocationID, ResourceID> allocationIdToResourceId) {
